...
def open_chat(job_id, recipient_id):
    ...
    if request.method == 'POST':
        ...
        if body:
            new_msg = Message(sender_id=current_user.id, recipient_id=recipient_id if recipient_id != 0 else None, body=body)
            ...
            if is_ai_agent:
                ...
                ai_response = openrouter_ai.get_ai_response(f"المستخدم: {current_user.full_name}. السيرة: {cv_text[:500]}.\nالسؤال: {body}")
                bot_user = User.query.filter(User.username.ilike('%bot%')).first()
                db.session.add(Message(sender_id=bot_user.id if bot_user else 1, recipient_id=current_user.id, body=ai_response, is_read=True))
                db.session.commit()
    ...
    messages = Message.query.filter(((Message.sender_id == current_user.id) & (Message.recipient_id == target_id)) | 
                                    ((Message.sender_id == target_id) & (Message.recipient_id == current_user.id))).order_by(Message.timestamp.asc()).all()

    if request.args.get('json'):
        return jsonify({
            "is_online": (recipient.last_seen >= (datetime.utcnow() - timedelta(minutes=5))) if not is_ai_agent else False,
            "messages": [{"id": m.id, "body": m.body, "sender_id": m.sender_id} for m in messages[-10:]]
        })
