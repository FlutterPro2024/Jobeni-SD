...
from app.models import Message, User, Job, CV, db, Application # أضفنا Application و User
...
# --- الدالة الجديدة لربط زر إدارة المرشحين بالشات ---
@chat_bp.route('/start/<int:recipient_id>')
@login_required
def start_chat(recipient_id):
    """توجيه صاحب العمل لأول محادثة متاحة مع هذا المتقدم"""
    # البحث عن آخر وظيفة قدم فيها هذا المستخدم عند صاحب العمل الحالي
    ...
    # إذا وجدت تقديم، افتح الشات برقم الوظيفة، وإذا لم يوجد افتحه برقم 0 (شات عام)
...
def open_chat(job_id, recipient_id):
    ...
    if request.method == 'POST':
        ...
        if body or file_url:
            ...
            # ذكاء اصطناعي واشعارات (نفس كودك الأصلي)
            if is_ai_agent:
                ...
                bot_msg = Message(sender_id=bot_user.id if bot_user else 1, recipient_id=current_user.id, body=ai_response)
