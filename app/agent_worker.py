...
class JobeniAgent:
    ...
    def get_career_advice(query, cv_text=None):
        # توجيه البرومبت ليكون خبيراً عالمياً ومحلياً
        prompt = f"""
        أنت 'مساعد جوبيني الذكي'، مستشار مهني عالمي خبير في أسواق العمل الدولية (مثل الخليج، أوروبا، والعمل عن بعد) وكذلك السوق السوداني.
        المستخدم يسأل: {query}
        """
        if cv_text:
        ...
        prompt += """
        المطلوب منك:
        1. تقديم نصيحة مهنية شاملة تأخذ في الاعتبار المعايير العالمية والفرص المتاحة دولياً ومحلياً.
        2. إذا كانت مهارات المستخدم تسمح بالعمل عن بعد (Remote Work) أو الهجرة المهنية، وضح ذلك.
        3. استخدم لغة مهنية واضحة ومنظمة جداً.
        4. لا تستخدم رموزاً تقنية مثل [OUT] أو [START] في ردك.
        5. اجعل الرد منسقاً في نقاط واضحة.
        """
