...
import textwrap
import urllib.parse
from app.models import User, CV, db, Job, Application, AgentMemory
...
class JobeniAgent:
    ...
    def calculate_match_percentage(cv_text, job_title, job_desc):
        ...
        Job: {job_title} | Description: {job_desc[:500]}
        CV Content: {cv_text[:1200]}
        Return ONLY valid JSON: {{"percentage": 0-100, "missing": "list key missing skills", "is_fit": true/false}}
        """
        try:
            res = openrouter_ai.get_ai_response(prompt, temperature=0.1)
            match = re.search(r'\{.*\}', res, re.DOTALL)
            if match: return json.loads(match.group())
            return {"percentage": 65, "missing": "مهارات تقنية"}
        except:
            return {"percentage": 50, "missing": "تعذر التحليل الفني"}

# --- Routes (المسارات السيادية) ---

@agent_bp.route('/run-discovery')
def trigger_discovery():
    """رابط خارجي لتشغيل الاكتشاف التلقائي"""
...
def trigger_weekly_summary():
    """إرسال التقرير الأسبوعي للمستخدمين"""
...
@login_required
def get_certificate():
    """توليد وإرسال الشهادة الموثقة عبر تليجرام"""
    if not current_user.telegram_id:
        flash("يرجى ربط حساب تليجرام أولاً لتلقي الشهادة.", "warning")
    ...
    if cert_img:
        ...
        caption = (f"📜 *تهانينا {display_name}!*\n\n"
                   f"تم إصدار شهادتك الرسمية من *جوبيني السودان*.\n"
                   f"الشهادة موثقة بكود QR صالح للتحقق.")
        try:
            requests.post(url, data={'chat_id': current_user.telegram_id, 'caption': caption, 'parse_mode': 'Markdown'}, files=files, verify=False)
            flash("تم إرسال الشهادة الفخمة إلى تليجرام بنجاح!", "success")
        except Exception as e:
            flash(f"خطأ في الإرسال: {str(e)}", "danger")
    return redirect(url_for('auth.dashboard'))
...
def run_agent():
    """رادار الوظائف: المحرك الرئيسي للاكتشاف والإرسال (تليجرام + واتساب)"""
    try:
        # اختيار مستخدم عشوائي مفعل عنده الأيجنت لتوزيع الحمل
        ...
        search_results = serper_searcher.search_jobs(f"{profession} jobs remote or worldwide")
        target_jobs = search_results.get('jobs', [])[:10]
        
        processed_count = 0

        for j in target_jobs:
            # منع التكرار باستخدام الذاكرة أو جدول الوظائف
            job_obj = Job.query.filter_by(title=j['title'], company_name=j['company']).first()
            if not job_obj:
                job_obj = Job(title=j['title'], company_name=j['company'], location=j.get('location', 'Remote'),
                              description=f"فرصة مكتشفة: {j['link']}")
            ...
            if not Application.query.filter_by(user_id=user.id, job_id=job_obj.id).first():
                ...
                score = match.get('percentage', 60)
                
                # حفظ في قاعدة البيانات كسجل مقترح
                db.session.add(Application(user_id=user.id, job_id=job_obj.id, status='suggested',
                                          match_score=score, applied_at=datetime.utcnow()))
                
                processed_count += 1

                # إرسال تليجرام
                if user.telegram_id and processed_count <= 3:
                    msg = f"🎯 *وظيفة جديدة:*\n{j['title']}\n🏢 {j['company']}\n📊 مطابقة: {score}%"
                    kb = {"inline_keyboard": [[{"text": "🔗 الرابط", "url": j['link']}]]}
                    send_message(user.telegram_id, msg, reply_markup=kb)

                # إرسال واتساب (Whapi)
                if user.whatsapp_number and score >= 70:
                    wa_msg = (f"🎯 *فرصة مكنة يا {user.username}!*\n\n"
                              f"📌 *الوظيفة:* {j['title']}\n"
                              f"🏢 *الشركة:* {j['company']}\n"
                              f"📊 *المطابقة:* {score}%\n\n"
                              f"🔗 *الرابط:* {j['link']}\n\n"
                              f"🤖 مرسل بواسطة وكيل جوبيني الذكي")
                    send_whatsapp_via_whapi(user.whatsapp_number, wa_msg)

        db.session.commit()
        return f"Agent success. Found {processed_count} jobs for {user.username}.", 200
    except Exception as e:
        db.session.rollback()
