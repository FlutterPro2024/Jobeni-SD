...
def create_app(config_name='default'):
    ...
    with app.app_context():
        ...
        # استيراد وتسجيل الـ Blueprints الأساسية
        ...
        # استيراد وتسجيل الـ Blueprints الإضافية مع معالجة الأخطاء
        try:
            ...
            app.logger.error(f"فشل في تسجيل بعض الأجزاء غير الأساسية: {e}")

    # محمل المستخدم لـ Flask-Login
    @login_manager.user_loader
    def load_user(user_id):
        from app.models import User
        return db.session.get(User, int(user_id))

    # إضافة متغيرات عامة لجميع القوالب (Templates)
