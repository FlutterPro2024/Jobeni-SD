...
@login_required
def dashboard():
    """لوحة التحكم الذكية: تشمل رادار المهارات وتوصيات AI"""
    ...
    course_suggestions = "ارفع سيرتك الذاتية للحصول على توصيات AI."
    ...
    return render_template('dashboard.html', radar_labels=radar_labels, radar_scores=radar_scores,
                           course_suggestions=course_suggestions, user_qr=user_qr_base64,
                           agent_memories=agent_memories, weekly_stats=weekly_stats)

@auth_bp.route('/update_agent_settings', methods=['POST'])
@login_required
def update_agent_settings():
    """تحديث إعدادات الرادار وربط الواتساب"""
    try:
        ...
        if whatsapp:
            ...
            # إرسال رسالة ترحيب عبر الواتساب (اختياري)
            try:
                from app.agent_worker import send_whatsapp_via_whapi
                send_whatsapp_via_whapi(clean_wa, f"تم تفعيل رادار جوبيني بنجاح لـ {current_user.username} 🤖")
            except: pass

        db.session.add(AgentMemory(user_id=current_user.id, action='settings_updated', feedback_notes="تحديث الإعدادات"))
        db.session.commit()
...
def generate_interview_prep():
    ...
    last_cv = CV.query.filter_by(user_id=current_user.id).order_by(CV.created_at.desc()).first()
    if not last_cv: return jsonify({'status': 'error', 'message': 'ارفع CV أولاً'})
    try:
...
def force_upgrade():
    """إصلاح قاعدة البيانات في Vercel بإضافة الأعمدة الناقصة"""
    try:
