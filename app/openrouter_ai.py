...
class OpenRouterAI:
    def __init__(self):
        ...
        # --- 20 نموذجاً صغيراً وسريعاً (Small/Flash) لضمان الاستجابة الفورية ---
        self.small_models = [
            ...
            "jondurbin/airoboros-l2-7b", 
            "runtastic/llama-3-8b-instruct"
        ]

        # --- 10 نماذج متوسطة (Medium) للتوازن بين الدقة والسرعة ---
        self.medium_models = [
        ...
        # --- 10 نماذج كبيرة (Large/Heavy) للتحليلات العميقة جداً ---
        self.large_models = [
        ...
        # دمج الكل في محرك واحد (40 نموذجاً)
        self.models = self.small_models + self.medium_models + self.large_models

    def _call_ai(self, prompt, temperature=0.3):
        """المحرك الأساسي: يلف على الـ 40 نموذجاً حتى يجد واحداً متاحاً"""
        if not self.api_key:
        ...
        for model in self.models:
            try:
                ...
                # مهلة انتظار قصيرة (8 ثواني) لضمان الانتقال السريع بين النماذج المشغولة
    ...
    def get_ai_response(self, prompt, temperature=0.5):
        """دالة عامة للمحادثات والمستشار المهني"""
        return self._call_ai(prompt, temperature)

    def analyze_cv_complete(self, cv_text):
        """تحليل السيرة الذاتية لاستخراج البيانات المهنية"""
        prompt = f"""
        Act as a Recruiter. Analyze CV. Return ONLY JSON.
        Format: {{"skills": ["Skill1"], "profession": "Title", "overall_score": 80, "feedback": "Arabic Advice"}}
        Text: {cv_text[:3000]}
        """
        ...
        try:
            ...
            return {
                ...
                "profession": "متخصص", 
                "overall_score": 50, 
                "feedback": "تم التحليل الأولي بنجاح."
            }

    def generate_improved_text(self, cv_content):
        """تحسين السيرة الذاتية لنظام الـ ATS"""
        prompt = f"""
        Rewrite this resume professionally. 
        1. Use English for content.
        2. Use the bullet point symbol '•' for all lists.
...
# تصدير الكائن للاستخدام في باقي ملفات المنصة
